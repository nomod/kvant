class ParserController < ApplicationController
  ...
  def parser_site
    ...
    # #вынимаем все ссылки на товары со страницы
    # doc = Nokogiri::HTML(open('http://ksrzd.ru/catalog/materialy-dlya-stroitelstva-ks/stoyki-metallicheskie/'))
    ...
    #
    #     #пишем в базу
    #     Product.create(friendly_url: @friendly_url.downcase, category_id: 19, product_title: link.content, view_main: false, image: open('public/img/upload_product/product_no_img.jpg'), price: 0)
    #
    #     #пишем в файл
    #     File.open("#{Rails.root}/db/products.txt", 'a') { |file| file.write("
    #     {
    ...
    #   @last_product_id = Product.last.id
    #
    #   #массив с id аттрибутов для ригелей жестких поперечин: Температура эксплуатации (мин, C), Сталь, Наименование, Чертеж, Размеры, Масса, Тип
    #   @mass_ids = [63,14,3,64,65,15,66]
    #   @mass_ids2 = [63,14,3,64,15,66] #без размеров
    #
    #   #выбираем из характеристик только значения
    ...
    #     end
    #
    #     if @mass_params.size == 6
    #       #пишем в файл
    #       File.open("#{Rails.root}/db/params.txt", 'a') { |file| file.write("
    #       {
    #       product_id: '#{@last_product_id}',
    #       product_atrs_id: '#{@mass_ids2[index]}',
    #       value: '#{@value}',
    #       },") }
    #     end
    #
